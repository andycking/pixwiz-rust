...
use druid::{
    AppLauncher, Color, Data, ImageBuf, PlatformError, Point, Rect, Widget, WidgetExt, WindowDesc,
};
use std::default;
...
impl Palette {
    pub fn new() -> Self {
        let mut rgbs = [[0; 16]; 16];

        //let bytes = include_bytes!("./assets/vga.pal");

        let mut values: Vec<Vec<Color>> = Vec::new();
        ...
        for y in 0..16 {
            ...
            for x in 0..16 {
                let v = (VGA[i] << 8) | 0xff;
                values[y].push(Color::from_rgba32_u32(v));
                i += 1;
            }
        }
...
impl Widget<PixWizState> for Palette {
    ...
    fn paint(&mut self, ctx: &mut PaintCtx, data: &PixWizState, env: &Env) {
        for y in 0..16 {
            for x in 0..16 {
                let color = &self.values[y][x];
                let origin = Point::new(1.0 + (x as f64) * 11.0, 1.0 + (y as f64) * 11.0);
                let rect = Rect::from_origin_size(origin, (10.0, 10.0));
                ctx.fill(rect, color);
            }
        }
    }
