...
pub fn push_undo(data: &mut AppState, area: druid::Rect) {
    push_undo_inner(data, area);

    // Important: reset the redo stack!
    data.redo.clear();
}

fn push_undo_inner(data: &mut AppState, area: druid::Rect) {
...
pub fn pop_redo(data: &mut AppState) {
    match data.redo.pop() {
        Some(record) => {
            ...
            push_undo_inner(data, record.area);
